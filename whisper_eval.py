...
import os
import scipy.wavfile.io as swi
...
if denoise:
    ...
    def prepare_dataset(batch):
        ...
        audio = batch["audio"]
        swi.write("temp.wav", audio["array"], rate=16000)

        est_sources = model.separate_file(path="test.wav")
        arr = torchaudio.functional.resample(est_sources[:, :, 0].detach().cpu(), orig_freq=8000, new_freq=16000)

        # compute log-Mel input features from input audio array
        batch["input_features"] = feature_extractor(arr, sampling_rate=audio["sampling_rate"]).input_features[0]
